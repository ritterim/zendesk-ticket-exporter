...
namespace ZendeskTicketExporter.Core
{
    ...
    public class TicketExporter : ExporterBase<TicketResultFlattened>
    {
        ...
        public override async Task RefreshLocalCopyFromServer(bool newDatabase = false)
        {
            ...
            while (true)
            {
                var pageForDisplay = nextPage.ToString();// PageForDisplay(nextPage);
                _log.InfoFormat("Begin copying tickets using page {0}.", pageForDisplay);
                ...
                nextPage = Convert.ToInt32(batch. NextPage);
